...
make_option(c("--out_directory"), type="character", default=NULL,
  help="This is the out directory", action="store"),
...
make_option(c("--total_budget"), type="character", default=500000,
  help="This is the total budget", action="store"),
make_option(c("--python_path"), type="character", default=500000,
  help="This is the total budget", action="store")
)
...
python_path <- gsub('"', '', args$python_path)
...
# setwd(args$upload_folder)

filename <- gsub('"', '', args$file)
dt_data <-read.csv(filename, header = TRUE, sep = ",", stringsAsFactors = FALSE)
paid_media_spends <- unlist(strsplit(gsub('"', '', args$paid_media_spends),","), use.names=FALSE)
paid_media_vars <- unlist(strsplit(gsub('"', '', args$paid_media_vars),","), use.names=FALSE)
organic_vars <- unlist(strsplit(gsub('"', '', args$organic_vars),","), use.names=FALSE)
factor_vars <- NULL
window_start <- gsub('"', '', args$start_date)
window_end <- gsub('"', '', args$end_date)
context_vars <- unlist(strsplit(gsub('"', '', args$context_vars),","), use.names=FALSE)
joined_objectives <- unlist(list(gsub('"', '', paid_media_spends), organic_vars,factor_vars), use.names=FALSE)
date_field <- gsub('"', '', args$date_field)
dep_var <- gsub('"', '', args$dep_var)
dep_var_type <- gsub('"', '', args$dep_var_type)
country_code <- gsub('"', '', args$country_code)
robyn_out_directory_with_id <-  gsub('"', '', args$out_directory)
...
colnames(export_folder)[1] <- 'export_folder'
...
allocator_max_response_fn <- function(model) {
  print(paste0('Running for Model: ', model))
  AllocatorCollect1 <- robyn_allocator(
    InputCollect = InputCollect,
    OutputCollect = OutputCollect,
    select_model = gsub('"', '', model),
